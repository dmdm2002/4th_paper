...
class Train:
    ...
    def train_one_epoch(self, epoch, train_score):
        ...
        domain_loss_weight = get_lambda(epoch, self.cfg['epoch'])
        for _, (x_source, x_target, y) in enumerate(
            ...
            # source_domain_label = torch.zeros(x_source.shape[0]).long().to(self.cfg['device'])
            # target_domain_label = torch.ones(x_source.shape[0]).long().to(self.cfg['device'])

            label_logit_source,  dense_feature_source_list = self.model(x_source)
            label_logit_target,  dense_feature_target_list = self.model(x_target)
            ...
            # source_domain_loss = self.loss_ce(domain_logit_source, source_domain_label)
            # target_domain_loss = self.loss_ce(domain_logit_target, target_domain_label)
            # domain_loss = source_domain_loss + target_domain_loss

            dense_distribution_loss = dist_matching_loss(dense_feature_source_list, dense_feature_target_list, loss_type='Bhattacharyya')

            total_loss = (label_loss * 1) + (dense_distribution_loss * 0.01)
            ...
            label_loss_avg += label_loss
            distribution_loss_avg += dense_distribution_loss * 0.01
